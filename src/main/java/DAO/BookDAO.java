...
public class BookDAO {
	...
	public int insertBook(Book book) throws SQLException, ClassNotFoundException {
		...
		int result = 0;
		String insert = "INSERT INTO Book (name, category_id, amount, image, tacgia, nhaxuatban) VALUES (?,?, ?, ?, ?, ?)";
		...
		preSt.setString(5, book.getTacgia());
		preSt.setString(6, book.getNhaxuatban());
...
//		String sql = "Select * from Book where name like '%l';";
	...
	public int updateBook(Book book) throws SQLException, ClassNotFoundException {
		...
		if (conn == null)
			conn = ConnectDatabase.getMySQLConnection();
		String sql = "Update Book set name =?,category_id =?,amount =?,image =?,tacgia =?, nhaxuatban =?  where id=? ";
		PreparedStatement pstm = (PreparedStatement) conn.prepareStatement(sql);
		...
		pstm.setString(7, book.getNhaxuatban());
